# 🌸 TipsCode de Yasmine

Bienvenue dans mon recueil personnel de **tips et notes de code**.  
Chaque fichier regroupe les **tips et notes de code** que j’utilise au quotidien dans mes projets.💻
Je les partage ici sur GitHub au cas où cela pourrait aider quelqu’un d’autre 💛  

> ⚠️ Ce n’est pas une documentation officielle !  
> J’update les fichiers de manière très personnelle et aléatoire, selon mes besoins du moment 😄  

---

## 📚 Table des matières

| Dossier | Description |
|----------|--------------|
| [angular](./angular/tips-angular.md) | Commandes & best practices Angular |
| [css](./css/tips-css.md) | Sélecteurs, animations, astuces responsives |
| [docker](./docker/tips-docker.md) | Commandes Docker & Dockerhub |
| [git](./git/tips-git-github.md) | Git, GitHub & workflow |
| [html](./html/tips-html.md) | Bonnes pratiques HTML |
| [javascipt](./javascipt/tips-javascript.md) | Syntaxe, fonctions utiles, ES6+ |
| [node](./node/tips-node.md) | Node.js & modules |
| [php](./php/tips-php.md) | Syntaxe, outils, exemples |
| [sql](./sql/tips-sql.md) | Requêtes, clés et jointures |
| [symfony](./symfony/tips-symfony.md) | Commandes et structures |
| [tailwind](./tailwind/tips-tailwindcss.md) | Classes utilitaires & layout |
| [typescript](./typescript/tips-typescript.md) | Typage, interfaces, génériques |

---

> ✨ By [Yasmine Badarou], n'hésitez pas à visiter mon portfolio (https://yasmine-badarou-portfolio.netlify.app)
